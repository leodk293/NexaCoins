...
export default function Page() {
    ...
    async function getSearchedCrypto() {
        ...
        try {
            const response = await fetch(`https://api.coingecko.com/api/v3/coins/${encodeURIComponent(cryptoName)}`, options);
    ...
    return (
        <main className="m-auto w-auto md:w-[60rem]">
            ...
            {crypto.loading
                ...
                crypto.error
                    ?
                    <p className=" text-2xl h-[10rem] font-bold text-red-600 text-center mt-10">Something went wrong...</p>
                    :
                    (crypto && crypto.data) ?
                        (
                            crypto &&
                            <div className=' mt-10 flex flex-col'>
...

                                {
                                    crypto.data.description.en ?
                                        <div className=' mt-2 text-white text-[15px] leading-9 md:text-[18px]'>
                                            <ReadMore
                                                text={(crypto.data.description.en).replace(/<a[^>]*>(.*?)<\/a>/gi, '$1')}
                                                maxLength={400}
                                            />
                                        </div>
                                        :
                                ...
                                <div className=' text-white mt-5 text-[18px] flex flex-col gap-2 md:text-xl'>
                                    ...
                                    <div className=' flex flex-row justify-between'>
                                        ...
                                        <p className=' font-semibold'>{currency.symbol} {(crypto.data.market_data.current_price[`${currency.name}`]).toLocaleString('en-US')}</p>
                                    </div>
                                    ...
                                    <div className=' flex flex-row justify-between'>
                                        ...
                                        <p className=' font-semibold'>{currency.symbol} {(crypto.data.market_data.market_cap[`${currency.name}`]).toLocaleString('en-US')}</p>
                                    </div>
                                    ...
                                    <div className=' flex flex-row justify-between'>
                                        ...
                                        <p className=' font-semibold'>{currency.symbol} {(crypto.data.market_data.high_24h[`${currency.name}`]).toLocaleString('en-US')}</p>
                                    </div>
                                    ...
                                    <div className=' flex flex-row justify-between'>
                                        ...
                                        <p className=' font-semibold'>{currency.symbol} {(crypto.data.market_data.low_24h[`${currency.name}`]).toLocaleString('en-US')}</p>
                                    </div>
                                ...
                                <p className=' mt-[70px] text-white text-[15px] font-semibold md:text-xl'>{crypto.data.name}&apos;s official website : <Link className=' font-bold text-orange-700 hover:text-orange-900 duration-200' target='_blank' href={crypto.data.links.homepage[0]}>{crypto.data.links.homepage[0]}</Link></p>
                            </div>
                        )
                        :
                        (
