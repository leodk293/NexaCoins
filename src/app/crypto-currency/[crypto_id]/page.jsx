...
export default function Page({ params }) {
  ...
  useEffect(() => {
    params.then((resolvedParams) =>
      setCryptoCurrency_id(resolvedParams.crypto_id)
    );
  }, [params]);
  ...
  async function fetchCoinsData() {
    ...
    try {
      const response = await fetch(
        `https://api.coingecko.com/api/v3/coins/${cryptoCurrency_id}`,
        options
      );
          ...
          <div className=" mt-10 flex flex-col">
            ...
            {coinsData.data.description.en ? (
              <div className=" mt-2 text-white text-[15px] leading-9 md:text-[18px]">
                <ReadMore
                  text={coinsData.data.description.en.replace(
                    /<a[^>]*>(.*?)<\/a>/gi,
                    "$1"
                  )}
                  maxLength={400}
            ...
            <div className=" text-white mt-5 text-[18px] flex flex-col gap-2 md:text-xl">
              <div className=" flex flex-row justify-between">
                ...
                <p className=" font-semibold">
                  {currency.symbol}{" "}
                  {coinsData.data.market_data.current_price[
                    `${currency.name}`
              ...
              <div className=" flex flex-row justify-between">
                ...
                <p className=" font-semibold">
                  {currency.symbol}{" "}
                  {coinsData.data.market_data.market_cap[
                    `${currency.name}`
                  ].toLocaleString("en-US")}
                </p>
              </div>
              ...
              <div className=" flex flex-row justify-between">
                ...
                <p className=" font-semibold">
                  {currency.symbol}{" "}
                  {coinsData.data.market_data.high_24h[
                    `${currency.name}`
                  ].toLocaleString("en-US")}
                </p>
              </div>
              ...
              <div className=" flex flex-row justify-between">
                ...
                <p className=" font-semibold">
                  {currency.symbol}{" "}
                  {coinsData.data.market_data.low_24h[
                    `${currency.name}`
                  ].toLocaleString("en-US")}
                </p>
              </div>
            ...
            <p className=" mt-[70px] text-white text-[15px] font-semibold md:text-xl">
              {coinsData.data.name}&apos;s official website :{" "}
              <Link
                className=" font-bold text-orange-700 hover:text-orange-900 duration-200"
                target="_blank"
                href={coinsData.data.links.homepage[0]}
              >
                {coinsData.data.links.homepage[0]}
              </Link>
            </p>
          </div>
        )
